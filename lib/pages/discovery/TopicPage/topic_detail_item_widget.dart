                ...
                Row(
                  children: <Widget>[
                    Text(
                      ...
                      style:
                      TextStyle(color: MyColor.hitTextColor, fontSize: 12),
                    ),
                    Expanded(
                      child: Text(
  ...
  Widget _description() {
    ...
    return Padding(
      ...
      child: ExpandMoreTextWidget(
        ...
        style: TextStyle(fontSize: 14, color: MyColor.desTextColor),
        moreStyle: textStyle,
        lessStyle: textStyle,
      ),
    );
    ...
    List<Widget> widgetList = itemData.data.content.data.tags.map((tag) {
      return Container(
        ...
        decoration: BoxDecoration(
            color: MyColor.tabBgColor, borderRadius: BorderRadius.circular(4)),
        child: Text(
          tag.name,
  ...
  Widget _videoState() {
    return Row(
      ...
      children: <Widget>[
        ...
        Row(
          children: <Widget>[
            Icon(Icons.favorite_border, size: 20, color: MyColor.hitTextColor),
            Padding(
              padding: EdgeInsets.only(left: 10),
              child: Text(
                  '${model.data.content.data.consumption.collectionCount}',
                  style: TextStyle(fontSize: 12, color: MyColor.hitTextColor)),
            )
          ],
        ),
        // 评论
        Row(
          children: <Widget>[
            Icon(Icons.message, size: 20, color: MyColor.hitTextColor),
            Padding(
              padding: EdgeInsets.only(left: 10),
        ...
        Row(
          children: <Widget>[
            Icon(Icons.star_border, size: 20, color: MyColor.hitTextColor),
            Padding(
              padding: EdgeInsets.only(left: 10),
              child: Text(MyString.collect_text,
                  style: TextStyle(fontSize: 12, color: MyColor.hitTextColor)),
            ),
          ],
        ),
        // 分享
        IconButton(
          icon: Icon(Icons.share, color: MyColor.hitTextColor),
